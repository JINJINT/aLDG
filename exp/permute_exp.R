...
#run_simu_batch<-function(i, start, marginal, cordiff){
#    start = 20*start
#    print(i)
#    filename = paste0(marginal, '_linear',cordiff,'_m',i,'_')
#    dat = readRDS(paste0('./dat/dat_', filename,'.rds'))

#    X = dat[[1]]
#    Y = dat[[2]]

#    X[is.infinite(X)] = max(X[!is.infinite(X)])
#    Y[is.infinite(Y)] = max(Y[!is.infinite(Y)])

#    sLED_all( X, Y, methods = c('pearson','dcor','taustar','hsic','csn'),
#             abs = FALSE, norm = FALSE,  npermute=20,
#             extrainfo = filename,
#             start = start, save = FALSE)

#}

#run_simu_batch(as.integer(args[1]), as.integer(args[2]), as.character(args[3]), as.character(args[4]))


run_batch_permutation<-function(nperm){
  for(i in c(1,2,3,4)){
    datafilelist = c('nowvel_celltypeL4_50','0.1nowvel_celltypeL23_50','0.2nowvel_celltypeL23_50','0.3nowvel_celltypeL23_50')
    ...
    sLED_one(X, Y, nperm, permidx, methods = c('pearson','taustar','dcor','hsic','hoeffd','ssd'),
...
#}
    ...
    #   permlist =  readRDS(paste0('./dat/permidx_',permfilelist[i],'.rds'))
    #   if(nperm==0){
    #     permidx = list(i1 = 1:ncol(X), i2 = (ncol(X)+1):ncol(Y))
    #   }else{
    #     permidx = permlist[[nperm]]
    #   }
    #   print(str(permidx))
    ...
    #   #X = 2^X -1
    #   #Y = 2^Y -1
    #   cormat_x = compute_corrmat(X, methods = c('pearson','taustar','hsic','hoeffd','csn'),
    #                              thred = 0, abs = FALSE, norm = FALSE, ncores=10,
    #                              extrainfo = paste0(datafilelist[i],'X1'))
    #
    #   cormat_y = compute_corrmat(Y, methods =  c('pearson','taustar','hsic','hoeffd','csn'),
    #                              thred = 0, abs = FALSE, norm = FALSE, ncores=10,
...
#load('./dat/Velme_asd_gene.RData')
#load("~/Desktop/CSNproj/code/dat/Velme_metacell_cpm.RData")
...
#velasd = mc.cpm[asd.genes, which(colnames(mc.cpm) %in% celltypelistlist[4] & subtype.mc$diagnosis=='ASD')]
#velctl = mc.cpm[asd.genes, which(colnames(mc.cpm) %in% celltypelistlist[4] & subtype.mc$diagnosis=='Control')]
...
# velasdmarker = velasd[velmarkers$genes[c(1:100, 701:895)],]
# velctlmarker = velctl[velmarkers$genes[c(1:100, 701:895)],]
