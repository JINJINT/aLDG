...
# for(k in 1:10){
#   for(i in c(1,2,3,4)){
#     dat = readRDS(paste0('./realdat/',datafilelist[i],'.rds'))
#     for(a in c(0.1,0.3,0.5)){
#       newdat = dat
#       tochange = floor(a*ncol(newdat[[1]]))
#       to1 = sample(1:ncol(newdat[[1]]),tochange)
#       to2 = sample(1:ncol(newdat[[2]]),tochange)
#       new1 = newdat[[1]][,to1]
#       new2 = newdat[[2]][,to2]
#       newdat[[1]][,to1] = new2
#       newdat[[2]][,to2] = new1
#       saveRDS(newdat, paste0('./realdat/',a,datafilelist[i],'_trial',k,'.rds'))
#     }
#   }
# }
# 
...
datafilelist = '0.1nowvel_celltypeL23_50'
ans = c()
permfilelist = 'vel_log_L23'
filename = paste0('./realdat/',datafilelist,'.rds')
dat = readRDS(filename)
X = log2(dat[[1]]+1)
Y = log2(dat[[2]]+1)

filelist = list.files("./dat/dat/dat",
                      pattern=paste0('sledstat_',datafilelist,'allallplot_trial1.*.rds'),
                      full.names = TRUE)

ans = sLED_all(X, Y, npermute=200, methods = c('pearson', 'spearman', 'kendall', 'taustar', 'dcor', 'hsic', 'hoeffd', 'mic','ssd'),
               thred=0, abs = FALSE, norm = FALSE,
               extrainfo = paste0(datafilelist,'_try2'),
               filelist=filelist[2:201])$pVal
