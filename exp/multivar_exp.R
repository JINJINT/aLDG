...
run_batch_permutation<-function(nperm){
  datafile = 'nowvel_celltypeL23_50'
  for(k in c(1:10)){
        permfile = 'vel_log_L23'
      
        filename = paste0('./realdat/0.1',datafile,'_trial',k,'.rds')
        dat = readRDS(filename)
        
        X = log2(dat[[1]]+1)
        Y = log2(dat[[2]]+1)
        
        permlist =  readRDS(paste0('./realdat/permidx_',permfile,'.rds'))
        if(nperm==0){
        ...
        sLED_one(X, Y, nperm, permidx, methods =  c('Pearson','aLDG'), 
                 ...
                 extrainfo = paste0(datafile,'_aldgwd1_trial',k))
      }
}
...
for(k in 1:10){
  ...
  filelist = list.files("./dat",
                        pattern=paste0('sledstat_nowvel_celltypeL23_50_aldgwd1_trial',k,'_.*.rds'),
                        full.names = TRUE)
  print(length(filelist))
  ans = cbind(ans,sLED_all(X, Y, npermute=200, methods = c('Pearson', 'aLDG'),
                 thred=0, abs = FALSE, norm = FALSE,
                 extrainfo = paste0('nowvel_celltypeL23_50_aldgwd1_trial',k),
