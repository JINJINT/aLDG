...
#' @export
getDiffMatrix<-function(X,Y,methods,thred=-Inf,info=0, norm = FALSE, abs = FALSE,extrainfo='',dir='./',ncores=NULL){
  Dhat_list = list()
  if(length(methods)>0){
    CX <- compute_corrmat(X, methods = methods,
                          thred = thred, info=info,
                          norm = norm, abs = abs,
                          extrainfo =paste0(extrainfo,'X'),
                          dir=dir,ncores=ncores)
    CY <- compute_corrmat(Y, methods = methods,
                          thred = thred, info=info,
                          norm = norm, abs = abs,
                          extrainfo =paste0(extrainfo,'Y'),
                          dir=dir,ncores=ncores)
    methods = names(CX)
  ...
  for(i in 1:length(methods)){
    Dhat_list[[i]][is.infinite(Dhat_list[[i]])]=0
    Dhat_list[[i]][is.na(Dhat_list[[i]])]=0
  }
  return(Dhat_list)
}
...
sLED_all <- function(X, Y, thred = -Inf, methods = c('pearson'),
  ...
  for(j in 1:npermute){
    filename = paste0('~/Desktop/dat/sledstat_', extrainfo,'_',j,'.rds')
    if(!file.exists(filename)){
      ...
      Tn_permute_list <- sLEDpermute(Z=Z, n1=n1, n2=n2, rho=rho,thred = thred,
                                     ...
                                     useMC=useMC, mc.cores=mc.cores, seeds=seeds, save= save,
  ...
  #if(!save){
  #  Tn_list = readRDS(filenamenull)
  #  Tn_permute_list = readRDS(filename)
  #  pVal_list = sapply(names(Tn_list), function(method)rowSums(Tn_permute_list[[method]] > Tn_list[[method]]) / npermute)
  #  return(list(pVal = pVal_list, Tn = Tn_list, Tn_permute = Tn_permute_list))
  #}
}
